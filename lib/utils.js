...
function watchTokens(tokenList, onTokensUpdated, bulkTokenUpdateType) {
    let url = `${window.overlay.config.getWs()}/tokens`;
    if(bulkTokenUpdateType){
        url += `?bulkUpdates=${bulkTokenUpdateType}`
    }
    let rws = new ReconnectingWebSocket(url, null, {
        automaticOpen: false,
